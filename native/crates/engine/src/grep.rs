...
//!
//! Wraps `cauldron_grep` functions and exposes them as JS-callable N-API exports.
...
// ── N-API types (mirroring cauldron_grep types for the JS boundary) ────────
...
fn convert_context_line(cl: cauldron_grep::ContextLine) -> NapiContextLine {
    NapiContextLine {
...
fn convert_search_match(m: cauldron_grep::SearchMatch) -> NapiSearchMatch {
    NapiSearchMatch {
...
fn convert_file_match(m: cauldron_grep::FileMatch) -> NapiGrepMatch {
    NapiGrepMatch {
        path: m.path,
        line_number: clamp_u32(m.line_number),
        line: m.line,
...
pub fn search(content: Buffer, options: NapiSearchOptions) -> Result<NapiSearchResult> {
    let opts = cauldron_grep::SearchOptions {
    ...
    match cauldron_grep::search_content(content.as_ref(), &opts) {
...
pub fn grep(options: NapiGrepOptions) -> task::Async<NapiGrepResult> {
    task::blocking("grep", (), move |_ct| {
        let opts = cauldron_grep::GrepOptions {
            pattern: options.pattern,
            path: options.path,
            glob: options.glob,
            ignore_case: options.ignore_case.unwrap_or(false),
        ...
        match cauldron_grep::search_path(&opts) {
