//! Cauldron `.cauldron/` directory file parser.
//!
//! Parses markdown files containing YAML-like frontmatter, section headings,
//! and structured content used by Cauldron's planning system (roadmaps, plans,
...
//! - `batchParseCauldronFiles`: walk a `.gsd/` tree and parse all `.md` files in parallel
...
/// A single parsed Cauldron file from batch parsing.
#[napi(object)]
...
/// Batch-parse all `.md` files in a `.cauldron/` directory tree.
///
...
#[napi(js_name = "batchParseCauldronFiles")]
pub fn batch_parse_cauldron_files(directory: String) -> Result<BatchParseResult> {
...
// ─── Cauldron Tree Scanner ───────────────────────────────────────────────────────

#[napi(object)]
pub struct CauldronTreeEntry {
    pub path: String,
    pub name: String,
    #[napi(js_name = "isDir")]
    pub is_dir: bool,
...
pub fn scan_cauldron_tree(directory: String) -> Result<Vec<CauldronTreeEntry>> {
...
fn collect_tree_entries(base: &Path, dir: &Path, entries: &mut Vec<CauldronTreeEntry>) -> Result<()> {
    ...
    for entry in read_dir {
        ...
        entries.push(CauldronTreeEntry {
            path: relative,
            name,
            is_dir,
        });
